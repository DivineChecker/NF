...
# Model options selectable via /switchmodel button.
# NOTE: claude.ai's internal API expects specific model slugs that don't
# always match public API names. If a slug is rejected (HTTP 400
# "Unsupported model"), the bot automatically retries without specifying
# a model at all, falling back to the account's default model.
MODEL_OPTIONS = {
    "sonnet46": {"label": "Sonnet 4.6", "slug": "claude-sonnet-4-6"},
    "sonnet5":  {"label": "Sonnet 5",   "slug": "claude-sonnet-5"},
}
...
/switchmodel — Switch between Sonnet 4.6 / Sonnet 5
...
@bot.message_handler(commands=["switchmodel"])
@auth_check
def cmd_switchmodel(msg: Message):
    us = get_session(msg.from_user.id)

    kb = InlineKeyboardMarkup()
    buttons = []
    for key, opt in MODEL_OPTIONS.items():
        active = " ✅" if us.model == opt["slug"] else ""
        buttons.append(InlineKeyboardButton(f"{opt['label']}{active}", callback_data=f"model_{key}"))
    kb.row(*buttons)

    bot.reply_to(
        msg,
        f"🤖 <b>Choose a model</b>\n\nCurrent: <code>{html_lib.escape(us.model)}</code>",
        parse_mode="HTML",
        reply_markup=kb,
    )
...
def cb_switchmodel(call: CallbackQuery):
    uid = call.from_user.id
    us  = get_session(uid)

    key = call.data[len("model_"):]
    opt = MODEL_OPTIONS.get(key)

    if not opt:
        bot.answer_callback_query(call.id, "Unknown model option.")
        return

    us.model = opt["slug"]
    # Force a new conversation so the new model takes effect on the next message
    us.conversation_id = ""

    bot.answer_callback_query(call.id, f"Switched to {opt['label']}")

    try:
        bot.edit_message_text(
            f"✅ <b>Model switched to {opt['label']}</b>\n\n"
            f"<code>{html_lib.escape(opt['slug'])}</code>\n\n"
            f"<i>A new conversation will start with your next message.\n"
            f"If this model slug isn't supported by your account, the bot "
            f"will automatically fall back to your account's default model.</i>",
            chat_id=call.message.chat.id, message_id=call.message.message_id,
            parse_mode="HTML",
        )
    except Exception:
    ...
    log.info(f"User {uid} switched model to {opt['slug']}")
...
def main():
    try:
        bot.set_my_commands([
            ...
            BotCommand("switchmodel",   "Switch between Sonnet 4.6 / Sonnet 5"),
